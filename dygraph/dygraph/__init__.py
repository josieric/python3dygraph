...
def dygraph(df, df_opts, divname = "graphdiv", includejs = True):
  dygraph="<div id=\"" +divname+ "\" style=\"margin: 0 auto; width:auto;\"></div><script type=\"text/javascript\">"
  if (includejs):
    dygraph += "\n" + jscode
  dygraph += "\nvar  "+divname+" = new Dygraph(document.getElementById('" +divname+ "'),"
  dygraph += "\""+ df.to_csv(index=True,line_terminator="\\n") +"\""+ "," + str(df_opts) + ");\n</script>"
  return dygraph

def dygraphSS(df, df_opts, divname = "graphdiv", includejs = True):
  dygraph="<div id=\"" +divname+ "\" style=\"margin: 0 auto; width:auto;\"></div><div id=\"" +divname+ "_sel\" align=\"center\"></div><script type=\"text/javascript\">"
  if (includejs):
    dygraph += "\n" + jscode
  dygraph += "\nvar  "+divname+" = new Dygraph(document.getElementById('" +divname+ "'),"
  dygraph += "\""+ df.to_csv(index=True,line_terminator="\\n") +"\""+ "," + str(df_opts) + ");\n"
